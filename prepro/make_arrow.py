...
def path2rest_octa(path, iid2captions, iid2split, iid2label):
    name = path
    with open(path, "rb") as fp:
        binary = fp.read()
    captions = iid2captions[name]
    split = iid2split[name]
    label_path = iid2label[name]
    with open(label_path, "rb") as fp:
        label_binary = fp.read()
    return [binary, captions, name, split, label_binary]

def make_arrow_octa500(data, dataset_name, save_dir):
    print(f"+ Pre-processing {dataset_name}...")
    # {路径： [texts]}
    iid2captions = defaultdict(list)
    # {路径：split}，比如'data/finetune_data/octa500/OCTA(FULL)/10201.bmp': 'test'
    iid2split = dict()
    iid2label = dict()
    ...
    for split, split_data in data.items():
        for sample in split_data:
            ...
            iid2label[sample["img_path"]] = sample["label_path"]
    ...
    bs = [path2rest_octa(path, iid2captions, iid2split, iid2label) for path in tqdm(caption_paths)]

    for split in ["train", "val", "test"]:
        # 倒数第二个是split！
        batches = [b for b in bs if b[-2] == split]
        dataframe = pd.DataFrame(batches, columns=["image", "caption", "image_id", "split", "label"])
