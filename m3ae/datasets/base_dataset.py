...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def get_label(self, index, image_key="label"):
        image = self.get_raw_image(index, image_key=image_key)
        # print("============label===============")
        # print(np.array(image))
        # print(np.array(image).shape)
        ...
        image_size = 400
        # 1 * 400 * 400 (灰度，单通道)
        trans = keys_to_transforms(["clip_transform_without_norm"], size=image_size)
        image_tensor = [tr(image) for tr in trans]
    ...
    def get_suite(self, index):
        ...
        while result is None:
            try:
                ...
                # 图片 label 仅octa有 todo：加一个开关（通过self.table去判断是否存在对应的key），预训练的时候要去掉！
                # labellabel
