...
from ..gadgets.my_metrics import Accuracy, Scalar, VQARADScore, SegMetrics
    ...
    for split in ["train", "val"]:
        for k, v in pl_module.hparams.config["loss_names"].items():
            ...
            if k == "vqa":
                ...
                if split == "train":
                    setattr(pl_module, f"train_{k}_accuracy", SegMetrics())
                    setattr(pl_module, f"train_{k}_loss", Scalar())
                else:
                    setattr(pl_module, f"val_{k}_accuracy", SegMetrics())
                    setattr(pl_module, f"val_{k}_loss", Scalar())
                    setattr(pl_module, f"test_{k}_accuracy", SegMetrics())
                    setattr(pl_module, f"test_{k}_loss", Scalar())
...
def epoch_wrapup(pl_module, test=False):
    ...
    for loss_name, v in pl_module.hparams.config["loss_names"].items():
        ...
        if loss_name == "vqa":
            ...
            acc, se, sp = getattr(pl_module, f"{phase}_{loss_name}_accuracy").compute()
            pl_module.log(f"{loss_name}/{phase}/accuracy_epoch", acc)
            pl_module.log(f"{loss_name}/{phase}/Se_epoch", se)
            pl_module.log(f"{loss_name}/{phase}/Sp_epoch", sp)
            getattr(pl_module, f"{phase}_{loss_name}_accuracy").reset()
...
def set_schedule(pl_module):
    ...
    head_names = ["mlm_head", "mim_head", "itm_head", "vqa_head", "cls_head", "irtr_head", "seg_head_conv", "seg_head_linear"]
