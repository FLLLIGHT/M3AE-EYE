...
def compute_itm(pl_module, batch):
    ...
    # 对同一个 batch 中的多幅图像进行匹配或不匹配组合处理
    # 执行的操作是：读入多个 image 和 false_image_0 并对它们做组合，
    # 套在一个 stack 函数中使其变成带通道维度（认为所有输入都是灰色图）
    itm_images = [
        torch.stack(
            [
    ...
    # 使用 Multi-Modal classifier 计算分类结果，并计算交叉熵损失
...
def compute_seg(pl_module, batch, test=False):
    infer = pl_module.infer(batch, mask_text=False, mask_image=False)

    # 目前的seg_logits shape : 1(batch size) * 400(height) * 400(width)
    # print(infer["multi_modal_cls_feats"].shape)
    # infer["multi_modal_seg_feats"].shape: 1, 754, 768
    # print(infer["multi_modal_seg_feats"].shape)
    seg_feats = infer["multi_modal_seg_feats"].view(1, 1, 754, 768)

    # bs: batch size
    seg_logits = pl_module.seg_head_conv(seg_feats)
    print(seg_logits.shape)
    seg_logits = seg_logits.view(1, 72192)
    seg_logits = pl_module.seg_head_linear(seg_logits)
    print(seg_logits.shape)
    # 暂时直接取第一个元素，batch size>1时需要调整！(todo)
    seg_labels = batch["label"][0][0].squeeze().reshape(1, -1) # .long()
    seg_logits = seg_logits
    # print(batch["label"][0][0])
    print( torch.unique(seg_logits, return_counts=True))
    print( torch.unique(seg_labels, return_counts=True))
    # print(set(batch["label"][0][0].squeeze().item()))
    print(seg_logits)
    print(seg_labels)
    print(seg_logits.shape)
    ...
    # 先用MSE，后面看DICE Loss
    # seg_loss = F.mse_loss(seg_logits, seg_labels)

    loss_fct = torch.nn.BCEWithLogitsLoss(reduction='mean')
    BCE_loss = loss_fct(seg_logits, seg_labels)
    ...
    # loss = getattr(pl_module, f"{phase}_seg_loss")(ret["seg_loss"])
    acc, se, sp = getattr(pl_module, f"{phase}_seg_accuracy")(ret["seg_logits"], ret["seg_labels"])
    # pl_module.log(f"seg/{phase}/loss", loss)
    pl_module.log(f"seg/{phase}/accuracy", acc)
    pl_module.log(f"seg/{phase}/Se", se)
    pl_module.log(f"seg/{phase}/Sp", sp)
